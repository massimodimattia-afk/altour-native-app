...
import {
  ShieldCheck, LogOut, Plus, ChevronRight, ChevronLeft,
  Gift, User, Award, Trophy, X, MapPin, CheckCircle2,
  Mountain, Camera,
...
const BADGE_NAMES: Record<string, string> = {
  ...
  Formazione: 'Sapiente', 'Immersi nel verde': 'Guardiano del Verde',
  'Luoghi dello spirito': 'Cercatore di Luce', Novità: 'Esploratore', Speciali: 'Leggenda',
  'Tra mare e cielo': 'Navigatore', 'Trek urbano': 'Flaneur',
  'Tracce sulla neve': 'Segugio della Neve', 'Cielo stellato': 'Astronomo',
};
...
interface EscursioneCompletata {
  titolo: string; colore: string; data: string;
  categoria?: string; difficolta?: string;
}
interface UserTessera {
  id: string; codice_tessera: string; nome_escursionista: string;
  cognome_escursionista: string; pin: string; avatar_url?: string;
  escursioni_completate: EscursioneCompletata[] | string;
  livello?: string; badges_filosofia?: string[] | string;
  km_totali?: number; dislivello_totali?: number;
}
...
function parseJsonArray<T>(data: T[] | string | null | undefined): T[] {
  if (typeof data === 'string') { try { return JSON.parse(data) as T[]; } catch { return []; } }
...
const ACHIEVEMENT_BADGES = [
  {
    id: 'streak_tour', name: 'Lupo dei Cammini', emoji: '🐺',
    description: '3 tour completati', color: '#e94544',
    check: (e: EscursioneCompletata[]) => e.filter(x => x.categoria === 'tour').length >= 3,
    progress: (e: EscursioneCompletata[]) => ({ current: Math.min(e.filter(x => x.categoria === 'tour').length, 3), total: 3 }),
  },
  {
    id: 'assiduo', name: 'Assiduo', emoji: '🎯',
    description: '8 giornate completate', color: '#01aa9f',
    check: (e: EscursioneCompletata[]) => e.filter(x => x.categoria === 'giornata').length >= 8,
    progress: (e: EscursioneCompletata[]) => ({ current: Math.min(e.filter(x => x.categoria === 'giornata').length, 8), total: 8 }),
  },
  {
    id: 'collezionista', name: 'Collezionista', emoji: '💎',
    description: '5 filosofie diverse', color: '#946a52',
    check: (e: EscursioneCompletata[]) => new Set(e.map(x => getFilosofiaName(x.colore)).filter(Boolean)).size >= 5,
    progress: (e: EscursioneCompletata[]) => ({ current: Math.min(new Set(e.map(x => getFilosofiaName(x.colore)).filter(Boolean)).size, 5), total: 5 }),
  },
  {
    id: 'stagionale', name: 'Anima delle Stagioni', emoji: '🍂',
    description: 'Tutte e 4 le stagioni', color: '#75c43c',
    check: (e: EscursioneCompletata[]) => new Set(e.map(x => getSeason(new Date(x.data)))).size >= 4,
    progress: (e: EscursioneCompletata[]) => ({ current: Math.min(new Set(e.map(x => getSeason(new Date(x.data)))).size, 4), total: 4 }),
  },
  {
    id: 'esploratore_verticale', name: 'Esploratore Verticale', emoji: '⚡',
    description: 'Facile → Media → Impegnativa', color: '#002f59',
    check: (e: EscursioneCompletata[]) => {
      const d = e.map(x => x.difficolta || '');
      return d.some(x => x === 'Facile') && d.some(x => x.includes('Media')) && d.some(x => x.includes('Impegnativa'));
    },
    progress: (e: EscursioneCompletata[]) => {
      const d = e.map(x => x.difficolta || '');
      let n = 0;
      if (d.some(x => x === 'Facile')) n++;
      if (d.some(x => x.includes('Media'))) n++;
      if (d.some(x => x.includes('Impegnativa'))) n++;
      return { current: n, total: 3 };
    },
  },
];

// ─── FIX 1: nome corretto e width numerica per logo ───
function IconaScarponeCustom({ size = 24, color = '#d6d3d1', isActive = false }: { size?: number; color?: string; isActive?: boolean }) {
  return (
    <Image
      source={require('../../assets/scarpone.png')}
      style={{ width: size, height: size, tintColor: isActive ? color : '#d6d3d1', opacity: isActive ? 1 : 0.15 }}
      resizeMode="contain"
    />
  );
}

// ─── FIX 2: ProgressBar con flex invece di width% stringa ───
function ProgressBar({ pct, color }: { pct: number; color: string }) {
  return (
    <View style={{ width: '100%', height: 8, backgroundColor: '#e7e5e4', borderRadius: 4, overflow: 'hidden' }}>
      <View style={{ flex: pct / 100, backgroundColor: color, height: 8, borderRadius: 4 }} />
    </View>
  );
}
    ...
    <TouchableOpacity onPress={onPress} activeOpacity={0.7}>
      <View style={s.badgeChipContainer}>
        <View style={s.badgeChipWrapper}>
          ...
          <View style={[s.badgeChipCard, isUnlocked
            ? { backgroundColor: color, shadowColor: color, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8 }
            : { backgroundColor: '#f7f6f4' }]}
          >
            <Text style={{ fontSize: isUnlocked ? 26 : 22, opacity: isUnlocked ? 1 : 0.22 }}>{emoji}</Text>
            {!isUnlocked && (
              <View style={{ width: 38, height: 3, backgroundColor: '#e0ddd8', borderRadius: 1.5, marginTop: 2, overflow: 'hidden' }}>
                <View style={{ width: `${pct}%` as any, height: 3, backgroundColor: color, borderRadius: 1.5 }} />
              </View>
            )}
          </View>
          {isUnlocked && (
            <View style={[s.badgeChipCheckmark, { borderColor: color }]}>
        ...
        <Text style={[s.badgeChipName, { color: isUnlocked ? color : '#c4c2c0' }]} numberOfLines={2}>{name}</Text>
      </View>
    </TouchableOpacity>
  );
}
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={s.modalOverlay}>
        <View style={s.popupContainer}>
          <TouchableOpacity onPress={onClose} style={s.popupCloseBtn}>
            <X size={16} color="#78716c" />
          </TouchableOpacity>
          <View style={[s.popupHeader, { backgroundColor: isUnlocked ? color : '#f0eeec' }]}>
            <Text style={{ fontSize: 56, opacity: isUnlocked ? 1 : 0.3 }}>{emoji}</Text>
          </View>
          <View style={s.popupBody}>
            <Text style={[s.popupStatus, { color: isUnlocked ? color : '#c4c2c0' }]}>
              {isUnlocked ? 'Badge Sbloccato ✓' : 'Badge Bloccato'}
            </Text>
...
function PinInput({ value, onChange, onComplete, length = 6, disabled }: { value: string; onChange: (v: string) => void; onComplete?: () => void; length?: number; disabled?: boolean }) {
  ...
  const handleKeyPress = (index: number, key: string) => {
    ...
    if (key === 'Backspace') {
      if (value[index]) {
        const p = value.split(''); p[index] = ''; onChange(p.join(''));
      } else if (index > 0) {
        const p = value.split(''); p[index - 1] = ''; onChange(p.join(''));
  ...
  return (
    <View style={s.pinContainer}>
      {/* Prima riga: cifre 0-2 */}
      <View style={s.pinRow}>
        {Array.from({ length: 3 }).map((_, i) => (
          <TextInput
            ref={el => { inputRefs.current[i] = el; }}
            style={[s.pinInput, focusedIndex === i && { borderColor: '#0ea5e9' }]}
            keyboardType="number-pad" maxLength={1} value={value[i] || ''}
            onChangeText={t => handleChange(i, t)}
            onKeyPress={({ nativeEvent }) => handleKeyPress(i, nativeEvent.key)}
            onFocus={() => setFocusedIndex(i)} onBlur={() => setFocusedIndex(null)}
            editable={!disabled} secureTextEntry
          />
        ))}
      </View>
      {/* Seconda riga: cifre 3-5 */}
      <View style={s.pinRow}>
        {Array.from({ length: 3 }).map((_, i) => {
          const idx = 3 + i;
          return (
            <TextInput
              ...
              style={[s.pinInput, focusedIndex === idx && { borderColor: '#0ea5e9' }]}
              keyboardType="number-pad" maxLength={1} value={value[idx] || ''}
...
export default function Tessera() {
  ...
  const escursioniCompletateParsed = useMemo(
    () => parseJsonArray<EscursioneCompletata>(userTessera?.escursioni_completate), [userTessera?.escursioni_completate]
  );
  const badgesFilosofiaParsed = useMemo(
    () => parseJsonArray<string>(userTessera?.badges_filosofia), [userTessera?.badges_filosofia]
  );
  ...
  const earnedBadges = useMemo(
    () => userTessera ? computeEarnedBadges(escursioniCompletateParsed) : [],
    [userTessera, escursioniCompletateParsed]
  );
  ...
  useEffect(() => {
    const loadSession = async () => {
      ...
      if (saved) { const { code } = JSON.parse(saved); await fetchUser(code, true); }
  ...
  async function fetchUser(codice: string, isSession = false) {
    ...
    const { data, error } = await supabase.from('tessere')
      .select('*, km_totali, dislivello_totali')
  ...
  // ─── FIX 3: Avatar upload senza FileReader ───
  const pickAndUploadAvatar = async () => {
    if (!userTessera) return;
    const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (status !== 'granted') { Alert.alert('Permesso negato', "Serve l'accesso alla galleria."); return; }
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images,
      allowsEditing: true, aspect: [1, 1], quality: 0.8,
      base64: true, // ← chiediamo base64 direttamente
    });
    if (!result.canceled && result.assets[0]) {
      ...
      try {
        ...
        // ─── FIX: usiamo base64 da ImageManipulator, niente FileReader ───
        const base64Data = manipResult.base64;
        if (!base64Data) throw new Error('Base64 non disponibile');
        ...
        await supabase.from('tessere').update({ avatar_url: freshUrl } as any).eq('id', userTessera.id);
  ...
  const closeRedeem = useCallback(() => {
    ...
    setShowRedeem(false); setRedeemCode(''); setRedeemStep('INPUT');
    setRedeemError(''); setPendingActivity(null);
    setNewlyUnlockedBadge(null); setRedeemAttempts(0);
  }, [isVerifying]);
  ...
  const verifyCode = async () => {
    ...
    const esc = data as any;
    if ((esc.codici_usati as string[] | null)?.includes(normalized)) { setRedeemError('Codice già usato.'); setIsVerifying(false); return; }
    if (escursioniCompletateParsed.some(e => e.titolo === esc.titolo)) { setRedeemError('Già riscattato.'); setIsVerifying(false); return; }
    const color = getFilosofiaColor(esc.filosofia);
    const newEntry: EscursioneCompletata = {
      titolo: esc.titolo, colore: color, data: new Date().toISOString(),
      ...(esc.categoria ? { categoria: esc.categoria } : {}),
      ...(esc.difficolta ? { difficolta: esc.difficolta } : {}),
    };
    ...
    setPendingActivity({ titolo: esc.titolo, filosofia: esc.filosofia ?? null });
    ...
    return (
      <View style={[s.loginContainer, { paddingTop: insets.top }]}>
        ...
        <View style={s.loginCard}>
          {/* ─── FIX 4: width numerica per il logo ─── */}
          ...
          {loginStep === 'code' ? (
            <>
              <TextInput placeholder="ALTXXX" value={loginCode} onChangeText={setLoginCode}
                autoCapitalize="characters" style={s.loginInput} />
              <TouchableOpacity onPress={() => fetchUser(loginCode)} style={s.loginBtn}>
                <Text style={s.loginBtnText}>Avanti</Text>
              </TouchableOpacity>
            </>
          ) : (
            <>
              ...
              <TouchableOpacity onPress={() => { setLoginStep('code'); setLoginPin(''); setLoginError(''); }} style={{ marginTop: 16 }}>
                <Text style={s.backLink}>Indietro</Text>
              </TouchableOpacity>
            </>
          )}
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.heroContainer}>
        <Image source={{ uri: 'https://rpzbiqzjyculxquespos.supabase.co/storage/v1/object/public/Images/Trentino_neve.webp' }}
          style={StyleSheet.absoluteFillObject} resizeMode="cover" />
        <View style={s.heroOverlay} />
        <TouchableOpacity onPress={handleLogout} style={[s.logoutBtn, { top: insets.top + 8 }]}>
          <LogOut size={18} color="white" />
        </TouchableOpacity>
        <View style={[s.heroContent, { paddingBottom: 24 }]}>
          ...
          <View style={s.heroBadge}>
            <IconaScarponeCustom size={50} color="#5aaadd" isActive />
            <View>
            ...
            <View style={s.profileCard}>
              ...
              <View style={s.profileRow}>
                <TouchableOpacity onPress={pickAndUploadAvatar} disabled={avatarUploading}>
                  <View style={s.avatarContainer}>
                    {userTessera.avatar_url
                      ? <Image source={{ uri: userTessera.avatar_url }} style={s.avatar} />
                      : <View style={s.avatarPlaceholder}><User size={32} color="#a8a29e" /></View>}
                ...
                <View style={s.profileInfo}>
                  <View style={s.verifiedBadge}>
                    ...
                    <Text style={s.verifiedText}>Escursionista Verificato</Text>
                  </View>
                  <Text style={s.profileName}>{userTessera.nome_escursionista} {userTessera.cognome_escursionista}</Text>
                  <Text style={s.profileLevel}>{userTessera.livello || stats?.currentLevelLabel} · {escursioniCompletateParsed.length} Scarponi</Text>
                </View>
              </View>

              {/* Stats */}
              <View style={s.statsRow}>
                <View style={[s.statBox, { backgroundColor: '#f0f9ff' }]}>
                  <Text style={{ fontSize: 28 }}>👣</Text>
                  <Text style={s.statLabel}>Km Totali</Text>
                  <Text style={s.statValue}>{stats?.kmTotali} km</Text>
                </View>
                <View style={[s.statBox, { backgroundColor: '#ecfdf5' }]}>
                  <Mountain size={32} color="#10b981" />
                  <Text style={s.statLabel}>Dislivello</Text>
                  <Text style={s.statValue}>{stats?.dislivelloTotali} m</Text>
                </View>
              </View>
              ...
              <View style={s.bootsGrid}>
                {Array.from({ length: SLOTS_PER_PAGE }).map((_, i) => {
                  ...
                  const esc = escursioniCompletateParsed?.[idx];
                  return (
                    <TouchableOpacity
                      key={i}
                      onPress={() => esc && setSelectedBoot(esc)}
                      disabled={!esc}
                      style={[s.bootSlot, esc ? s.bootSlotFilled : s.bootSlotEmpty]}
                    >
                      <IconaScarponeCustom size={width < 768 ? 44 : 60} color={esc?.colore} isActive={!!esc} />
                    </TouchableOpacity>
                  );
                })}
              </View>
              ...
              <View style={s.pagination}>
                <TouchableOpacity disabled={currentPage === 0} onPress={() => setCurrentPage(p => p - 1)}
                  ...
                  <ChevronLeft size={20} color={currentPage === 0 ? '#d6d3d1' : '#44403c'} />
                </TouchableOpacity>
                <Text style={s.pageIndicator}>Pagina {currentPage + 1} di {stats?.totalPages}</Text>
                <TouchableOpacity
                  disabled={currentPage >= (stats?.totalPages || 1) - 1}
                  onPress={() => setCurrentPage(p => p + 1)}
                  style={[s.pageBtn, currentPage >= (stats?.totalPages || 1) - 1 && s.pageBtnDisabled]}>
                  <ChevronRight size={20} color={currentPage >= (stats?.totalPages || 1) - 1 ? '#d6d3d1' : '#44403c'} />
                </TouchableOpacity>
              </View>
            </View>

            <TouchableOpacity onPress={() => setShowRedeem(true)} style={s.redeemBtn}>
              <Plus size={24} color="white" />
              <Text style={s.redeemBtnText}>Riscatta Scarpone</Text>
            </TouchableOpacity>

            {stats && stats.vouchersCount > 0 && (
              <View style={s.voucherBanner}>
                <Gift size={26} color="#d97706" />
                <View style={{ flex: 1 }}>
                  <Text style={s.voucherTitle}>VOUCHER SBLOCCATI 🎉</Text>
                  <Text style={s.voucherSubtitle}>{stats.vouchersCount} Voucher da 10 € maturati</Text>
                </View>
              </View>
            )}
          </View>
        ...
        {activeTab === 'BADGE' && (
          <View style={s.badgeGrid}>
            {Object.keys(BADGE_NAMES).map(filo => (
              <BadgeChip key={filo} filo={filo} isUnlocked={earnedBadges.includes(filo)}
                count={badgeCounts[filo] || 0}
                onPress={() => setSelectedBadge({ filo, isUnlocked: earnedBadges.includes(filo), count: badgeCounts[filo] || 0 })} />
            ))}
          </View>
        )}
        ...
        {activeTab === 'TRAGUARDI' && (
          <View style={s.achievementsContainer}>
            {ACHIEVEMENT_BADGES.map(badge => {
              ...
              return (
                <TouchableOpacity key={badge.id} onPress={() => setSelectedAchievement(badge)}
                  style={[s.achievementCard, isUnlocked && s.achievementCardUnlocked]}>
                  <Text style={s.achievementEmoji}>{badge.emoji}</Text>
                  <View style={s.achievementInfo}>
                    <Text style={s.achievementName}>{badge.name}</Text>
                    <Text style={s.achievementDesc}>{badge.description}</Text>
                  </View>
                  <View style={s.achievementProgress}>
                    <Text style={s.achievementProgressText}>{prog.current}/{prog.total}</Text>
                    <View style={s.achievementProgressBar}>
                      {/* ─── FIX 5: progress bar con flex ─── */}
                      <View style={{ flex: prog.current / prog.total, backgroundColor: badge.color, height: 4, borderRadius: 2 }} />
                    </View>
                  </View>
                </TouchableOpacity>
              );
          ...
          <View style={s.redeemModal}>
            ...
            {redeemStep === 'INPUT' ? (
              <>
                ...
                <TextInput placeholder="****" value={redeemCode} onChangeText={setRedeemCode}
                  style={s.redeemInput} autoCapitalize="characters" />
                {redeemError ? <Text style={s.redeemError}>{redeemError}</Text> : null}
                <TouchableOpacity onPress={verifyCode} disabled={isVerifying} style={s.redeemSubmitBtn}>
                  {isVerifying ? <ActivityIndicator color="white" /> : <Text style={s.redeemSubmitText}>Verifica Codice</Text>}
                </TouchableOpacity>
              </>
            ) : (
              <>
                ...
                {newlyUnlockedBadge && (
                  <View style={[s.unlockedBadge, { backgroundColor: FILOSOFIA_COLORS[newlyUnlockedBadge] + '15' }]}>
                    <Text style={{ fontSize: 20 }}>{BADGE_EMOJI[newlyUnlockedBadge]}</Text>
                    <Text style={{ fontWeight: '700', color: FILOSOFIA_COLORS[newlyUnlockedBadge] }}>
                      Nuovo Badge: {BADGE_NAMES[newlyUnlockedBadge]}
                    </Text>
                  </View>
                )}
                <TouchableOpacity onPress={closeRedeem} style={s.successBtn}>
                  <Text style={s.successBtnText}>Perfetto!</Text>
                </TouchableOpacity>
              </>
            )}
          </View>
      ...
      <Modal visible={!!selectedBoot} transparent animationType="fade" onRequestClose={() => setSelectedBoot(null)}>
        <View style={s.modalOverlay}>
          <View style={s.bootModal}>
            ...
            {selectedBoot && (
              <>
                <IconaScarponeCustom size={80} color={selectedBoot.colore} isActive />
                <Text style={s.bootModalTitle}>{selectedBoot.titolo}</Text>
                <Text style={s.bootModalDate}>
      ...
      {selectedBadge && (
        <BadgeDetailPopup visible={!!selectedBadge} {...selectedBadge} onClose={() => setSelectedBadge(null)} />
      )}
              ...
              {(() => {
                ...
                return (
                  <View style={{ width: '100%', marginTop: 20, alignItems: 'center' }}>
                    <Text style={{ fontWeight: '900', color: '#a8a29e', marginBottom: 8 }}>{prog.current}/{prog.total}</Text>
                    <View style={{ width: '100%', height: 4, backgroundColor: '#e7e5e4', borderRadius: 2, overflow: 'hidden' }}>
                      <View style={{ flex: prog.current / prog.total, height: 4, backgroundColor: '#0ea5e9', borderRadius: 2 }} />
                    </View>
                  </View>
                );
              })()}
...
const s = StyleSheet.create({
  ...
  // ─── FIX: width numerica, niente 'auto' ───
  loginLogo:          { height: 120, width: 240, marginBottom: 24, borderRadius: 12 },
  loginTitle:         { fontSize: 24, fontWeight: '900', textTransform: 'uppercase', marginBottom: 24, color: '#1c1917' },
  ...
  pinLabel:           { fontSize: 12, fontWeight: 'bold', color: '#a8a29e', textTransform: 'uppercase', marginBottom: 24, textAlign: 'center' },
  backLink:           { fontSize: 10, fontWeight: '900', textTransform: 'uppercase', color: '#d6d3d1' },
  loginError:         { marginTop: 16, color: '#ef4444', fontSize: 12, fontWeight: 'bold', textAlign: 'center' },
  heroContainer:      { height: 260, justifyContent: 'flex-end' },
  heroOverlay:        { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.35)' },
  logoutBtn:          { position: 'absolute', right: 16, padding: 10, backgroundColor: 'rgba(0,0,0,0.25)', borderRadius: 20, zIndex: 10 },
  heroContent:        { paddingHorizontal: 24, alignItems: 'center' },
  heroTitle:          { fontSize: 28, fontWeight: '900', color: 'white', textTransform: 'uppercase', letterSpacing: -1, textAlign: 'center' },
  heroName:           { fontSize: 18, fontWeight: '900', color: 'white', textTransform: 'uppercase', marginTop: 4 },
  heroCode:           { fontSize: 11, fontWeight: '700', color: 'rgba(255,255,255,0.7)', letterSpacing: 4, textTransform: 'uppercase', marginTop: 6, marginBottom: 14 },
  heroBadge:          { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: 'rgba(255,255,255,0.15)', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 30, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  heroBadgeLabel:     { fontSize: 9, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 3, color: 'rgba(255,255,255,0.6)' },
  heroBadgeLevel:     { fontSize: 13, fontWeight: '900', textTransform: 'uppercase', color: 'white' },
  tabBar:             { flexDirection: 'row', backgroundColor: '#f5f2ed', paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#e7e5e4' },
  tab:                { flex: 1, paddingVertical: 10, borderRadius: 20, alignItems: 'center' },
  tabActive:          { backgroundColor: '#5aaadd' },
  tabText:            { fontSize: 11, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1.5, color: '#a8a29e' },
  tabTextActive:      { color: 'white' },
  contentContainer:   { paddingHorizontal: 16, paddingBottom: 48 },
  tabContent:         { paddingTop: 20 },
  profileCard:        { backgroundColor: 'white', borderRadius: 32, padding: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.06, shadowRadius: 16, elevation: 4, marginBottom: 20 },
  ...
  profileName:        { fontSize: 17, fontWeight: '900', textTransform: 'uppercase', color: '#1c1917' },
  profileLevel:       { fontSize: 10, fontWeight: '700', color: '#a8a29e', textTransform: 'uppercase', letterSpacing: 1, marginTop: 2 },
  statsRow:           { flexDirection: 'row', gap: 12, marginBottom: 24 },
  statBox:            { flex: 1, alignItems: 'center', paddingVertical: 16, borderRadius: 20 },
  statLabel:          { fontSize: 9, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1.5, color: '#a8a29e', marginTop: 6 },
  statValue:          { fontSize: 18, fontWeight: '900', marginTop: 2, color: '#1c1917' },
  ...
  // ─── FIX: dashed solo su iOS, Android usa solid ───
  bootSlotEmpty:      { backgroundColor: '#fafafa', borderWidth: 1, borderColor: '#e7e5e4', opacity: 0.6 },
  ...
  pageIndicator:      { fontSize: 10, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2, color: '#a8a29e' },
  redeemBtn:          { backgroundColor: '#0ea5e9', borderRadius: 20, paddingVertical: 18, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 12, marginTop: 20, shadowColor: '#0ea5e9', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.3, shadowRadius: 12, elevation: 6 },
  ...
  voucherSubtitle:    { fontSize: 15, fontWeight: '900', color: '#78350f', marginTop: 4 },
  ...
  achievementEmoji:   { fontSize: 34, marginRight: 12 },
  achievementInfo:    { flex: 1 },
  achievementName:    { fontSize: 13, fontWeight: '900', textTransform: 'uppercase', marginBottom: 2, color: '#1c1917' },
  ...
  achievementProgressBar: { width: 48, height: 4, backgroundColor: '#e7e5e4', borderRadius: 2, marginTop: 4, overflow: 'hidden', flexDirection: 'row' },
  ...
  redeemTitle:        { fontSize: 20, fontWeight: '900', textTransform: 'uppercase', marginBottom: 20, color: '#1c1917' },
  ...
  successTitle:       { fontSize: 18, fontWeight: '900', marginTop: 12, color: '#1c1917' },
  ...
  bootModalTitle:     { fontSize: 18, fontWeight: '900', textTransform: 'uppercase', marginTop: 16, textAlign: 'center', color: '#1c1917' },
  ...
  achievementModalName: { fontSize: 20, fontWeight: '900', textTransform: 'uppercase', textAlign: 'center', color: '#1c1917' },
  achievementModalDesc: { fontSize: 12, color: '#a8a29e', textTransform: 'uppercase', letterSpacing: 1, marginTop: 4, textAlign: 'center' },
  badgeChipContainer: { alignItems: 'center', width: 80 },
  badgeChipWrapper:   { position: 'relative', marginBottom: 6 },
  ...
  badgeChipCounter:   { position: 'absolute', bottom: -8, alignSelf: 'center', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 10, backgroundColor: 'white', borderWidth: 1.5 },
  badgeChipCounterText: { fontSize: 8, fontWeight: '900' },
  badgeChipName:      { fontSize: 9, fontWeight: '900', textTransform: 'uppercase', textAlign: 'center', maxWidth: 72 },
  popupContainer:     { width: '100%', maxWidth: 320, backgroundColor: 'white', borderRadius: 40, overflow: 'hidden' },
  popupCloseBtn:      { position: 'absolute', top: 16, right: 16, zIndex: 10, padding: 8, backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: 20 },
